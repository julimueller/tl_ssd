...
class MultiBoxLossLayer : public LossLayer<Dtype> {
 public:
  ...
  //virtual inline int MinBottomBlobs() const { return 4;}
    ...
    // bottom[1] stores the confidence predictions.
    // bottom[2] stores the prior bounding boxes.
    // bottom[3] stores the state predictions.
 ...
 protected:
  ...
  // The internal state loss layer.
  shared_ptr<Layer<Dtype> > state_loss_layer_;
  ConfLossType state_loss_type_;
  float state_weight_;
  // bottom vector holder used in Forward function.
  vector<Blob<Dtype>*> state_bottom_vec_;
  // top vector holder used in Forward function.
  vector<Blob<Dtype>*> state_top_vec_;
  // blob which stores the confidence prediction.
  Blob<Dtype> state_pred_;
  // blob which stores the corresponding ground truth label.
  Blob<Dtype> state_gt_;
  // confidence loss.
  Blob<Dtype> state_loss_;


  MultiBoxLossParameter multibox_loss_param_;
  FocalLossParameter focal_loss_param_;
  int num_classes_;
