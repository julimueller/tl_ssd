...
void DetectionOutputLayer<Dtype>::Forward_gpu(
  ...
  const Dtype* state_cpu_data;

  if(do_state_prediction_) {

      Dtype *state_permute_data = state_permute_.mutable_gpu_data();

      PermuteDataGPU<Dtype>(bottom[3]->count(), bottom[3]->gpu_data(),
              num_states_, num_priors_, 1, state_permute_data);
      state_cpu_data = state_permute_.cpu_data();
  ...
  for (int i = 0; i < num; ++i) {
    ...
    if (!share_location_) {
      cur_bbox_data += (num_classes_-1) * num_priors_ * 4;
    }
    ApplyNMSFastAllClasses(cur_bbox_data, cur_conf_data, num_priors_,
        confidence_threshold_, nms_threshold_, eta_, top_k_, indices, num_classes_, background_label_id_);
  ...
  // For multilabel we want to get all class confidences
  if(do_state_prediction_) {
      return_number = 4 + 3 + num_states_ ;
      top_shape.push_back(return_number);

  }
  // Else we only want the highest confidence
  else {
      return_number = 7;
  ...
  if (num_kept == 0) {
    ...
    std::cout << "top0count" << top[0]->count() << std::endl;
  ...
  for (int i = 0; i < num; ++i) {
    ...
    int state_idx;
    if(do_state_prediction_) {
        state_idx = i * num_states_ * num_priors_;
    }
    else {
         state_idx = 0;
    }
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        int data_index = 0;
        top_data[count * return_number] = i;
        top_data[count * return_number + 1] = label;
        top_data[count * return_number + 2] = cur_conf_data[idx];
        data_index = 3;

        if (do_state_prediction_){

            for(size_t c = 0; c < num_states_; ++c){
              const Dtype* new_state_data = state_cpu_data + state_idx + c * num_priors_;
              top_data[count * return_number + 3 + c] = new_state_data[idx];
            }
            data_index += num_states_;
        }

        for (int k = 0; k < 4; ++k) {
          top_data[count * return_number + data_index + k] = cur_bbox_data[idx * 4 + k];
        }
        if (need_save_) {
          ...
          //  TODO: MULTILABEL cur_det.add_child("confidence_vector", conf_vec);
