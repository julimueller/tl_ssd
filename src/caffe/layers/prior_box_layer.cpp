...
void PriorBoxLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < prior_box_param.offset_w_size(); ++i) {
      CHECK_GE(prior_box_param.offset_w(i), 0.) << "prior offset should be larger than 0.";
      CHECK_LE(prior_box_param.offset_w(i), 1) << "prior offset should be smaller than 1.";
      offset_w_.push_back(prior_box_param.offset_w(i));
  }

  for (int i = 0; i < prior_box_param.offset_h_size(); ++i) {
      CHECK_GE(prior_box_param.offset_h(i), 0.) << "prior offset should be larger than 0.";
      CHECK_LE(prior_box_param.offset_h(i), 1) << "prior offset should be smaller than 1.";
      offset_h_.push_back(prior_box_param.offset_h(i));
  }
  num_priors_ = aspect_ratios_.size() * min_sizes_.size() * offset_w_.size() * offset_h_.size();
...
void PriorBoxLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int h = 0; h < layer_height; ++h) {
    for (int w = 0; w < layer_width; ++w) {
        // 0.0 <= offset_w || offset_h <= 1.0 original offset is only at center of feature cell, now we allow smaller step sizes to also catch small objects at later network layers
        for (int o_x = 0; o_x < offset_w_.size(); ++o_x) {
            for (int o_y = 0; o_y < offset_h_.size(); ++o_y) {
                float center_x = (w + offset_w_[o_x]) * step_w;
                float center_y = (h + offset_h_[o_y]) * step_h;
