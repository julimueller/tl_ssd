...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  num_states_ = detection_output_param.num_states();
  share_location_ = detection_output_param.share_location();
  do_state_prediction_ = detection_output_param.do_state_prediction();
  ...
  if (do_state_prediction_) {
      state_permute_.ReshapeLike(*(bottom[3]));
  }
}
...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (do_state_prediction_) {
      if (state_permute_.num() != bottom[3]->num() ||
              state_permute_.count(1) != bottom[3]->count(1)) {
          state_permute_.ReshapeLike(*(bottom[3]));
      }
  }
  ...
  if (do_state_prediction_) {
  CHECK_EQ(num_priors_ * num_states_, bottom[3]->channels())         
