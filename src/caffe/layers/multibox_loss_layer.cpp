...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do_state_prediction_ = multibox_loss_param.do_state_prediction();
  ...
  // Get number of states (Traffic Light specific)
  if (do_state_prediction_) {
  ...
  background_state_id_ = multibox_loss_param.background_state_id();
  ...
  state_weight_ = multibox_loss_param.state_weight();
  state_loss_type_ = multibox_loss_param.state_loss_type();
  state_bottom_vec_.push_back(&state_pred_);
  state_bottom_vec_.push_back(&state_gt_);
  state_loss_.Reshape(loss_shape);
  state_top_vec_.push_back(&state_loss_);

  if (state_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {

      CHECK_GE(background_state_id_, 0)
          << "background_state should be within [0, num_classes) for Softmax.";
      CHECK_LT(background_state_id_, num_states_)
          << "background_state should be within [0, num_classes) for Softmax.";
      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_softmax_state");
      ...
      // Fake reshape.
      vector<int> state_shape(1, 1);
      state_gt_.Reshape(state_shape);
      state_shape.push_back(num_states_);
      state_pred_.Reshape(state_shape);
      state_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
      state_loss_layer_->SetUp(state_bottom_vec_, state_top_vec_);

  }
  else if (state_loss_type_ == MultiBoxLossParameter_ConfLossType_LOGISTIC) {

      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_logistic_state");
      ...
      LOG(FATAL) << "We actually only support logistic loss for state prediction.";
  }
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    ...
    if(multibox_loss_param.use_focal_loss()) {
      ...
      layer_param.add_loss_weight(Dtype(1.));
      FocalLossParameter* fl_param = layer_param.mutable_focal_loss_param();
      fl_param->set_gamma(multibox_loss_param_.focal_loss_param().gamma());
      fl_param->set_alpha(multibox_loss_param_.focal_loss_param().alpha());
      layer_param.mutable_loss_param()->set_normalization(
          LossParameter_NormalizationMode_NONE);
      SoftmaxParameter* softmax_param = layer_param.mutable_softmax_param();
      softmax_param->set_axis(1);
...
void MultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (do_state_prediction_){
     num_state_predictions_ = bottom[4]->num();
  }
  CHECK_EQ(num_priors_ * num_states_, bottom[4]->channels())
      << "Number of priors must match number of state predictions.";
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* state_data;
  if (do_state_prediction_) {
       state_data = bottom[4]->cpu_data();
  }
  ...
  if(do_state_prediction_) {
      ...
      if (do_neg_mining_) {
        num_state_conf_ = num_matches_ + num_negs;
      } else {
        num_state_conf_ = num_ * num_priors_;
      }

      if(num_state_conf_ >= 1) {
          //Reshape the state data
          vector<int> state_shape;
          if (state_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
            state_shape.push_back(num_state_conf_);
            state_gt_.Reshape(state_shape);
            state_shape.push_back(num_states_);
            state_pred_.Reshape(state_shape);
          }
          else if (state_loss_type_ == MultiBoxLossParameter_ConfLossType_LOGISTIC) {
              state_shape.push_back(1);
              state_shape.push_back(num_state_conf_);
              state_shape.push_back(num_states_);
              state_gt_.Reshape(state_shape);
              state_pred_.Reshape(state_shape);
              ...
              LOG(FATAL) << "Unknown state loss type.";

          }

          if (!do_neg_mining_) {
              CHECK_EQ(state_pred_.count(), bottom[4]->count());
              state_pred_.ShareData(*(bottom[4]));
          }

          Dtype* state_pred_data = state_pred_.mutable_cpu_data();
          Dtype* state_gt_data = state_gt_.mutable_cpu_data();
          caffe_set(state_gt_.count(), Dtype(background_state_id_), state_gt_data);
          EncodeStatePrediction(state_data, num_, num_priors_, multibox_loss_param_,
                               all_match_indices_, all_neg_indices_, all_gt_bboxes,
                               state_pred_data, state_gt_data);
          state_loss_layer_->Reshape(state_bottom_vec_, state_top_vec_);
          state_loss_layer_->Forward(state_bottom_vec_, state_top_vec_);
      }
      else {
          state_loss_.mutable_cpu_data()[0] = 0;
  ...
  if (do_state_prediction_) {
      if (this->layer_param_.propagate_down(4)) {
        Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
            normalization_, num_, num_priors_, num_matches_);
        top[0]->mutable_cpu_data()[0] += state_weight_ * state_loss_.cpu_data()[0] / normalizer;
...
void MultiBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (do_state_prediction_) {
      ...
      if (propagate_down[4]) {
        Dtype* state_bottom_diff = bottom[4]->mutable_cpu_diff();
        caffe_set(bottom[4]->count(), Dtype(0), state_bottom_diff);
        if (num_state_conf_ >= 1) {
          vector<bool> state_propagate_down;
          // Only back propagate on prediction, not ground truth.
          state_propagate_down.push_back(true);
          state_propagate_down.push_back(false);
          state_loss_layer_->Backward(state_top_vec_, state_propagate_down,
                                     state_bottom_vec_);
          // Scale gradient.
          Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
              normalization_, num_, num_priors_, num_matches_);
          Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer;
          caffe_scal(state_pred_.count(), loss_weight,
                     state_pred_.mutable_cpu_diff());
          // Copy gradient back to bottom[1].
          const Dtype* state_pred_diff = state_pred_.cpu_diff();
          if (do_neg_mining_) {
            int count = 0;
            for (int i = 0; i < num_; ++i) {
              // Copy matched (positive) bboxes scores' diff.
              ...
              for (map<int, vector<int> >::const_iterator it =
                ...
                for (int j = 0; j < num_priors_; ++j) {
                  ...
                  // Copy the diff to the right place.
                  caffe_copy<Dtype>(num_states_,
                                    state_pred_diff + count * num_states_,
                                    state_bottom_diff + j * num_states_);
                  ++count;
                }
              }
              // Copy negative bboxes scores' diff.
              ...
              state_bottom_diff += bottom[4]->offset(1);
            }
          } else {
            // The diff is already computed and stored.
            bottom[4]->ShareDiff(state_pred_);
          }
        }
      }
