...
__global__ void ComputeConfLossKernel(const int nthreads,
    ...
    const int num_classes, const ConfLossType loss_type, const float focal_gamma, const float focal_alpha,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (loss_type == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
      ...
      for (int c = 0; c < num_classes; ++c) {
        ...
        // Compute softmax probability.
        Dtype prob = conf_data[start_idx + label];
        loss = -1.0 * focal_alpha * powf(1. - max(prob, Dtype(FLT_MIN)),focal_gamma) * log(max(prob, Dtype(FLT_MIN)));
    }*/
...
void ComputeConfLossGPU(const Blob<Dtype>& conf_blob, const int num,
  ...
  if (loss_type == MultiBoxLossParameter_ConfLossType_SOFTMAX /*|| loss_type == MultiBoxLossParameter_ConfLossType_FOCAL_LOSS*/) {
  ...
  ComputeConfLossKernel<Dtype><<<CAFFE_GET_BLOCKS(num_threads),
    CAFFE_CUDA_NUM_THREADS>>>(num_threads, conf_gpu_data, num_preds_per_class,
        num_classes, loss_type, focal_param.gamma(), focal_param.alpha(), match_blob.gpu_data(), conf_loss_gpu_data);
  // Save the loss.
  all_conf_loss->clear();
