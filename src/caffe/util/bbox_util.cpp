...
void MineHardExamples(const Blob<Dtype>& conf_blob,
  ...
  *num_negs = 0;
  ...
  ComputeConfLoss(conf_blob.cpu_data(), num, num_priors, num_classes,
      background_label_id, conf_loss_type, *all_match_indices, all_gt_bboxes, multibox_loss_param.focal_loss_param(),
      &all_conf_loss);
#else
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
      ...
      map<int, vector<NormalizedBBox> >* all_gt_bboxes, const bool &state_prediction) {
  all_gt_bboxes->clear();
  for (int i = 0; i < num_gt; ++i) {
    int start_idx = i * 8;
    int label, state;
    int state_digit_nr = 4;
    if (!state_prediction) {
...
void ComputeConfLoss(const Dtype* conf_data, const int num,
      const int num_preds_per_class, const int num_classes,
      const int background_label_id, const ConfLossType loss_type, const FocalLossParameter focal_param,
      vector<vector<float> >* all_conf_loss) {
  all_conf_loss->clear();
  for (int i = 0; i < num; ++i) {
...
void ComputeConfLoss(const Dtype* conf_data, const int num,
      ...
      const vector<map<int, vector<int> > >& all_match_indices,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,const FocalLossParameter focal_param,
      vector<vector<float> >* all_conf_loss) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int p = 0; p < num_preds_per_class; ++p) {
      ...
      if (loss_type == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
        ...
        Dtype prob = std::exp(conf_data[start_idx + label] - maxval) / sum;
        loss = -log(std::max(prob, Dtype(FLT_MIN)));
      } /*else if (loss_type == MultiBoxLossParameter_ConfLossType_FOCAL_LOSS) {
          float alpha = focal_param.alpha();
          float gamma = focal_param.gamma();
          CHECK_GE(label, 0);
          CHECK_LT(label, num_classes);
          // Compute softmax probability.
          // We need to subtract the max to avoid numerical issues.
          ...
          Dtype prob = std::exp(conf_data[start_idx + label] - maxval) / sum;
          loss = -1.0 * alpha * std::pow((1. - std::max(prob, Dtype(FLT_MIN))), gamma) * log(std::max(prob, Dtype(FLT_MIN)));
        }*/ else if (loss_type == MultiBoxLossParameter_ConfLossType_LOGISTIC) {
        int target = 0;
        for (int c = 0; c < num_classes; ++c) {
          if (c == label) {
        ...
        for (int j = 0; j < num_priors; ++j) {
          ...
          switch (conf_loss_type) {
            case MultiBoxLossParameter_ConfLossType_SOFTMAX:
              ...
              break;
           /* case MultiBoxLossParameter_ConfLossType_FOCAL_LOSS:
                conf_gt_data[idx] = gt_label;
                break;*/
...
template void EncodeConfPrediction(const double* conf_data, const int num,
      ...
      const vector<vector<int> >& all_neg_indices,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      double* state_pred_data, double* conf_gt_data);

template <typename Dtype>
void EncodeStatePrediction(const Dtype* state_data, const int num,
      const int num_priors, const MultiBoxLossParameter& multibox_loss_param,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<vector<int> >& all_neg_indices,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      Dtype* state_pred_data, Dtype* state_gt_data) {
  // CHECK_EQ(num, all_match_indices.size());
  // CHECK_EQ(num, all_neg_indices.size());
  // Retrieve parameters.
  CHECK(multibox_loss_param.has_num_states()) << "Must provide num_states.";
  const int num_classes = multibox_loss_param.num_states();
  CHECK_GE(num_classes, 1) << "num_classes should not be less than 1.";
  const int background_label_id = multibox_loss_param.background_state_id();
  const bool map_object_to_agnostic =
      multibox_loss_param.map_object_to_agnostic();
  if (map_object_to_agnostic) {
    if (background_label_id >= 0) {
  ...
  do_neg_mining = mining_type != MultiBoxLossParameter_MiningType_NONE;
  const ConfLossType conf_loss_type = multibox_loss_param.state_loss_type();
  int count = 0;
  for (int i = 0; i < num; ++i) {
    if (all_gt_bboxes.find(i) != all_gt_bboxes.end()) {
      // Save matched (positive) bboxes scores and labels.
      ...
      for (map<int, vector<int> >::const_iterator it =
        ...
        for (int j = 0; j < num_priors; ++j) {
          ...
          const int gt_label = map_object_to_agnostic ?
            background_label_id + 1 :
            all_gt_bboxes.find(i)->second[match_index[j]].state();
          int idx = do_neg_mining ? count : j;
          switch (conf_loss_type) {
            case MultiBoxLossParameter_ConfLossType_SOFTMAX:
              state_gt_data[idx] = gt_label;
              break;
           /* case MultiBoxLossParameter_ConfLossType_FOCAL_LOSS:
                state_gt_data[idx] = gt_label;
                break;*/
            case MultiBoxLossParameter_ConfLossType_LOGISTIC:
              state_gt_data[idx * num_classes + gt_label] = 1;
              break;
            default:
              LOG(FATAL) << "Unknown conf loss type.";
          }
          if (do_neg_mining) {
            // Copy scores for matched bboxes.
            caffe_copy<Dtype>(num_classes, state_data + j * num_classes,
                state_pred_data + count * num_classes);
            ++count;
          }
        }
      }
      ...
      if (do_neg_mining) {
        ...
        for (int n = 0; n < all_neg_indices[i].size(); ++n) {
          ...
          switch (conf_loss_type) {
            case MultiBoxLossParameter_ConfLossType_SOFTMAX:
              state_gt_data[count] = background_label_id;
              break;
           /* case MultiBoxLossParameter_ConfLossType_FOCAL_LOSS:
                conf_gt_data[count] = background_label_id;
            break;*/
            case MultiBoxLossParameter_ConfLossType_LOGISTIC:
              if (background_label_id >= 0 &&
                  background_label_id < num_classes) {
                state_gt_data[count * num_classes + background_label_id] = 1;
              }
              break;
            default:
              LOG(FATAL) << "Unknown conf loss type.";
    ...
    if (do_neg_mining) {
      state_data += num_priors * num_classes;
    } else {
      state_gt_data += num_priors;
    }
  }
}

// Explicite initialization.
template void EncodeStatePrediction(const float* state_data, const int num,
      const int num_priors, const MultiBoxLossParameter& multibox_loss_param,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<vector<int> >& all_neg_indices,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      float* data, float* state_gt_data);
template void EncodeStatePrediction(const double* state_data, const int num,
      const int num_priors, const MultiBoxLossParameter& multibox_loss_param,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<vector<int> >& all_neg_indices,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      double* state_pred_data, double* state_gt_data);
...
void GetPriorBBoxes(const Dtype* prior_data, const int num_priors,
  ...
  std::ofstream priorboxFile;
  priorboxFile.open("/home/muejul3/Desktop/priorboxes.txt");
  ...
  for (int i = 0; i < num_priors; ++i) {
    ...
    priorboxFile << prior_data[start_idx] << " " << prior_data[start_idx + 1] << " " << prior_data[start_idx + 2] << " " << prior_data[start_idx + 3] << "\n";
...
// Explicit initialization.
template bool SortScorePairDescendAllClasses(const pair<pair<float, int>, int>& pair1,
                                   const pair<pair<float, int>, int>& pair2);


template <typename T>
bool SortScorePairDescendAllClasses(const pair<pair<float, T>, int>& pair1,
                          const pair<pair<float, T>, int>& pair2) {
...
template <typename Dtype>
void GetMaxScoreIndexAllClasses(const Dtype* scores, const int num, const float threshold, const int num_classes, const int background_id,
      const int top_k, vector<pair<pair<Dtype, int>, int > >* score_index_class_vec) {
  // Generate index score pairs.
  for (int c = 0; c < num_classes; ++c) {
      ...
      const Dtype* scores_class = scores + c * num;

      for (int i = 0; i < num; ++i) {
        if (scores_class[i] > threshold) {
          pair<Dtype,int> pair_i = std::make_pair(scores_class[i], i);
          score_index_class_vec->push_back(std::make_pair(pair_i,c));
        }
      }

      // Sort the score pair according to the scores in descending order
      std::sort(score_index_class_vec->begin(), score_index_class_vec->end(),
                SortScorePairDescendAllClasses<int>);

      // Keep top_k scores if needed.
      if (top_k > -1 && top_k < score_index_class_vec->size()) {
        score_index_class_vec->resize(top_k);
      }
...
template <typename Dtype>
void ApplyNMSFastAllClasses(const Dtype* bboxes, const Dtype* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, map<int, vector<int> > &indices, int num_classes, int background_id) {
  // Get top_k scores (with corresponding indices).
  vector<pair<pair<Dtype, int>, int > > score_index_class_vec;
  GetMaxScoreIndexAllClasses(scores, num, score_threshold, num_classes, background_id, top_k, &score_index_class_vec);

  // Do nms.
  float adaptive_threshold = nms_threshold;
  indices.clear();
  while (score_index_class_vec.size() != 0) {
    const int idx = score_index_class_vec.front().first.second;
    const int class_id = score_index_class_vec.front().second;
    bool keep = true;
    ...
    for (int c = 0; c < num_classes; ++c) {
        ...
        for (int k = 0; k < indices[c].size(); ++k) {
          if (keep) {
            ...
            const int kept_idx =  indices[c][k];
    ...
    if (keep) {
      ...
      indices[class_id].push_back(idx);

    }

    score_index_class_vec.erase(score_index_class_vec.begin());
...
template
void ApplyNMSFastAllClasses(const float* bboxes, const float* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, map<int, vector<int> > &indices, int num_classes, int background_id);

template
void ApplyNMSFastAllClasses(const double* bboxes, const double* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, map<int, vector<int> > &indices, int num_classes, int background_id);
